    ...
    <header className="sticky top-0 z-20 w-full backdrop-blur bg-white/60 border-b border-black/5">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold tracking-tight text-slate-800 text-lg">{t("app.name")}</span>
        </div>
        <nav className="hidden md:flex items-center gap-6 text-sm text-slate-600">
          <a href="#weather" className="hover:text-slate-900 inline-flex items-center gap-1"><MapPin size={16}/>{t("nav.weather")}</a>
          <a href="#advisory" className="hover:text-slate-900 inline-flex items-center gap-1"><Leaf size={16}/>{t("nav.advisory")}</a>
        </nav>
